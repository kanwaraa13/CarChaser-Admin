...
export const TotalNumberSeller = () => {
  ...
  const handleClick = async (event) => {
	  ...
	  try {
		const response = await api.get(`/admin/total-number-of-car/TotalNumberOfSeller/${selectedYear}/${selectedMonth}`);
		setIsdisplay(true);
          ...
          <div className="monthy-report">
            <form>
              <div className="form-group">
                <label htmlFor="exampleFormControlSelect1" className="mr-3">Total number of Sellers in</label>
			  ...
			  {isdisplay && (
					<div>
					  ...
					  <h3>Total Number Of Seller Available In {displaymonth}, {displayyear}</h3><span>{displaynumber}</span>
					  </div>
					</div>
				)}
        </div>
