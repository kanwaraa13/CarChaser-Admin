...
export const TotalNumberCarAvailable = () => {
  ...
  const handleClick = async (event) => {
	  ...
	  try {
		const response = await api.get(`/admin/total-number-of-car/TotalNumberOfCarsAvailable/${selectedYear}/${selectedMonth}`);
		setIsdisplay(true);
          ...
          <div className="monthy-report">
            <form>
               <div className="form-group">
                ...
                <select className="form-control mx-2" id="exampleFormControlSelect1" required>
                  {months.map((month, index) => (
                    <option value={index} key={index}>{month}</option>
                  ))}
                </select>
                <select className="form-control mx-2" id="exampleFormControlSelect2" required>
                  <option>Year</option>
                  <option>2024</option>
                  <option>2025</option>
		   ...
		   {isdisplay && (
				<div>
				  ...
				  <h3>Total Number Of Cars Available In {displaymonth}, {displayyear}</h3><span>{displaynumber}</span>
				  </div>
				</div>
			)}
		</div>
